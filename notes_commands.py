...
COLLECTION = get_collection("notes_command_states")
...
async def _get_chat_states(chat_id: int) -> dict:
    ...
    merged = NOTES_COMMANDS.copy()
    merged.update({k: bool(v) for k, v in stored.items() if k in NOTES_COMMANDS})
    return merged
...
async def notes_manage_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "notes_manage_all_enable":
        ...
        reply_markup = await _build_notes_manage_keyboard(chat_id)
        await query.edit_message_text("✅ All notes commands enabled.", reply_markup=reply_markup)
        ...
        await _save_chat_states(chat_id, chat_states)
        reply_markup = await _build_notes_manage_keyboard(chat_id)
        await query.edit_message_text("❌ All notes commands disabled.", reply_markup=reply_markup)
